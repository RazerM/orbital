...
def lookahead(iterable, fillvalue=None):
    """Yields a tuple of the current item and the next item."""
    iterator = iter(iterable)
    exhausted = object()
    current = next(iterator, exhausted)

    while current is not exhausted:
        ahead = next(iterator, exhausted)
        if ahead is exhausted:
...
def uvw_from_elements(i, raan, arg_pe, f):
    ...
    W = np.array([sin_raan * sin_i, -cos_raan * sin_i, cos_i])
...
OrbitalElements = namedtuple("OrbitalElements", ["a", "e", "i", "raan", "arg_pe", "f"])
