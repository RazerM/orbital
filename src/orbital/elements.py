...
from .maneuver import Maneuver, Operation, PropagateAnomalyBy, PropagateAnomalyTo
...
class KeplerianElements:
    ...
    def __init__(
        self, a=None, e=0, i=0, raan=0, arg_pe=0, M0=0, body=None, ref_epoch=J2000
    ):
        self._a = a
        self.e = e
        self.i = i
    ...
    @classmethod
    def with_altitude(
        cls, altitude, body, e=0, i=0, raan=0, arg_pe=0, M0=0, ref_epoch=J2000
    ):
    ...
    @classmethod
    def with_period(
        cls, period, body, e=0, i=0, raan=0, arg_pe=0, M0=0, ref_epoch=J2000
    ):
        """Initialise orbit with a given period."""

        ke = cls(
            e=e, i=i, raan=raan, arg_pe=arg_pe, M0=M0, body=body, ref_epoch=ref_epoch
        )
    ...
    @classmethod
    def with_apside_altitudes(
        cls, alt1, alt2, i=0, raan=0, arg_pe=0, M0=0, body=None, ref_epoch=J2000
    ):
    ...
    @classmethod
    def with_apside_radii(
        cls, radius1, radius2, i=0, raan=0, arg_pe=0, M0=0, body=None, ref_epoch=J2000
    ):
    ...
    def M(self, value):
        warnings.warn(
            "Setting anomaly does not set time, use KeplerianElements"
            ".propagate_anomaly_to() instead.",
            OrbitalWarning,
        )
    ...
    def v(self):
        ...
        r_dot = (
            sqrt(self.body.mu / self.a) * (self.e * sin(self.f)) / sqrt(1 - self.e**2)
        )
        rf_dot = (
            sqrt(self.body.mu / self.a)
            * (1 + self.e * cos(self.f))
            / sqrt(1 - self.e**2)
        )
    ...
    def __getattr__(self, attr):
        ...
        if not attr.startswith("__"):
            for apoapsis_name in self.body.apoapsis_names:
                if attr == f"{apoapsis_name}_radius":
                    return self.apocenter_radius
                elif attr == f"{apoapsis_name}_altitude":
                    return self.apocenter_altitude
            for periapsis_name in self.body.periapsis_names:
                if attr == f"{periapsis_name}_radius":
                    return self.pericenter_radius
                elif attr == f"{periapsis_name}_altitude":
    ...
    def W(self):
        ...
        sin_i = sin(self.i)
        return np.array([sin(self.raan) * sin_i, -cos(self.raan) * sin_i, cos(self.i)])
    ...
    def __str__(self):
        return (
            ...
            f"    Semimajor axis (a)                           = {self.a / kilo:10.3f} km\n"
            f"    Eccentricity (e)                             = {self.e:13.6f}\n"
            f"    Inclination (i)                              = {degrees(self.i):8.1f} deg\n"
            f"    Right ascension of the ascending node (raan) = {degrees(self.raan):8.1f} deg\n"
            f"    Argument of perigee (arg_pe)                 = {degrees(self.arg_pe):8.1f} deg\n"
            f"    Mean anomaly at reference epoch (M0)         = {degrees(self.M0):8.1f} deg\n"
            f"    Period (T)                                   = {timedelta(seconds=self.T)}\n"
            f"    Reference epoch (ref_epoch)                  = {self.ref_epoch!s}\n"
            f"        Mean anomaly (M)                         = {degrees(self.M):8.1f} deg\n"
            f"        Time (t)                                 = {timedelta(seconds=self.t)}\n"
            f"        Epoch (epoch)                            = {self.epoch!s}"
        )
