...
from .utilities import lookahead, orbit_radius, saved_state, uvw_from_elements
...
class Plotter2D:
    ...
    def _plot_position(self, orbit, f=None, propagated=False, label=None):
        ...
        if propagated:
            ...
            label = f"Propagated position {self.propagate_counter}"
            self.propagate_counter += 1

        (pos_dot,) = self.axes.plot(pos[0], pos[1], "o", label=label)
    ...
    def _plot_body(self, orbit):
        ...
        self.axes.add_patch(
            Circle((0, 0), orbit.body.mean_radius / kilo, linewidth=0, color=color)
        )
...
class Plotter3D:
    ...
    def _plot_position(self, orbit, f=None, propagated=False, label=None):
        ...
        (pos_dot,) = self.axes.plot([x], [y], [z], "o", label=label)
